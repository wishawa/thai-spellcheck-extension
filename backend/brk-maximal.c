...
static BestBrk *    best_brk_new (int n_brk_pos, int n_inc_pos);
...
static BrkPool *    brk_root_pool (int pos_size, int inc_size, BrkEnv *env);
static int          brk_maximal_do_impl (const thwchar_t *ws, int len,
                                         const char *brkpos_hints,
                                         int pos[], size_t n,
                                         int inc[], size_t inc_n, int *inc_ret,
                                         BrkEnv *env);
...
brk_maximal_do (const thchar_t *s, int len, int pos[], size_t n, int inc[], size_t inc_n, int *inc_ret, BrkEnv *env)
{
    ...
    ret = brk_maximal_do_impl (ws, len, brkpos_hints, pos, n, inc, inc_n, inc_ret, env);
                     ...
                     BrkEnv *env)
{
    ...
    pool = brk_root_pool (n, inc_n, env);
    best_brk = best_brk_new (n, inc_n);
    ...
    *inc_ret = best_brk->cur_inc_pos;
    memcpy (inc, best_brk->inc_pos, *inc_ret * sizeof (inc[0]));
                 ...
                 BrkEnv *env)
{
    ...
    pool = brk_root_pool (recov_words, recov_words, env);
    ret = 0;
...
static BrkPool *
brk_root_pool (int pos_size, int inc_size, BrkEnv *env)
{
    ThBrk      *brk;
    BrkPool    *pool;
    BrkPool    *node;
    ...
    root_shot.inc_pos = NULL;
    root_shot.cur_inc_pos = 0;
    root_shot.n_inc_pos = inc_size;
...
brk_shot_init (BrkShot *dst, const BrkShot *src)
{
    ...
    dst->inc_pos = (int *) malloc (src->n_inc_pos * sizeof (int));
    memcpy (dst->inc_pos, src->inc_pos, src->cur_inc_pos * sizeof (int));
    dst->n_inc_pos = src->n_inc_pos;
    dst->cur_inc_pos = src->cur_inc_pos;
...
brk_shot_reuse (BrkShot *dst, const BrkShot *src)
{
    ...
    if (dst->n_inc_pos < src->n_inc_pos) {
        dst->inc_pos = (int *) realloc (dst->inc_pos,
                                        src->n_inc_pos * sizeof (int));
    }
    memcpy (dst->brk_pos, src->brk_pos, src->cur_brk_pos * sizeof (int));
    dst->n_brk_pos = src->n_brk_pos;
    dst->cur_brk_pos = src->cur_brk_pos;
...
static BestBrk *
best_brk_new (int n_brk_pos, int n_inc_pos)
{
    BestBrk *best_brk;
    ...
    best_brk->inc_pos = (int *) malloc ((size_t) n_inc_pos * sizeof (int));
    best_brk->n_inc_pos = n_inc_pos;
    best_brk->cur_inc_pos = 0;
...
best_brk_contest (BestBrk *best_brk, const BrkShot *shot)
{
           ...
           shot->cur_brk_pos <= best_brk->cur_brk_pos))))
    {
        ...
        memcpy (best_brk->inc_pos, shot->inc_pos,
                shot->cur_inc_pos * sizeof (int));
        best_brk->cur_inc_pos = shot->cur_inc_pos;
        best_brk->n_inc_pos = shot->n_inc_pos;
